...
fid=fopen('D:\学习相关\语音信号处理，阵列\实时语音处理实践指南\AEC-matlab_ok\near_16kHz_32f.pcm', 'rb'); % Load far end
ssin=fread(fid,inf,'float32');
fclose(fid);
%far is speaker played music
fid=fopen('D:\学习相关\语音信号处理，阵列\实时语音处理实践指南\AEC-matlab_ok\far_16KHz_32f.pcm', 'rb'); % Load fnear end
...
%%
threshold=0.5e-3;
echoBandRange = ceil(50/fs*N):floor(3000*2/fs*N);%回声频谱范围
...
shh=logspace(-1.3,-2.2,N+1)';%该值未使用
shh=ones(1,N+1)';
...
wins=[0;sqrt(hanning(2*N-1))];%用在功率计算上？？
ubufn=zeros(2*N,1);
ebuf=zeros(2*N,1);
ebuf2=zeros(2*N,1);
ebuf4=zeros(2*N,1);
...
%     erfb(pos:pos+N-1) = dk;
%     end
   erfb(pos:pos+N-1) = ekfb;%记录error数据
...
%   Ek2 = Ek ./(M*pn + 0.001); % Normalized error
    Ek2 = Ek ./(pn + 0.001); % Normalized error%归一化处理
    
    absEf = max(abs(Ek2), threshold);%对误差频谱进行门限限制，使之最大为门限值
...
%     end
    ...
    if (NLPon)  
        ...
        window = wins;
        gamma = 0.93;
        tmp = fft(xx.*window);%远端信号
        xf = tmp(1:N+1);
        tmp = fft(dd.*window);%近端信号
        df = tmp(1:N+1);
        tmp = fft(ee.*window);%对EE,XX,DD加窗进行FFT变换，加窗为了防止频谱泄露
...
%         Sd(end:1) = filter(freqSm, [1 -(1-freqSm)], Sd(end:1));
        ...
        cohedAvg(kk) = mean(cohed(echoBandRange));%平均计算各频段的相干性
        cohxd = real(Sxd.*conj(Sxd))./(Sx.*Sd + 1e-10);
        freqSm = 0.55;
        cohxd(2:end) = filter(freqSm, [1 -(1-freqSm)], cohxd(2:end));%滤波？？？？？
...
%ef = ef.*((weight*(min(1 - hnled)).^2 + (1 - weight).*(1 - hnled)).^2);
        ...
        [xSort, xSortIdx] = sort(Sx);%排序，取值及序列
...
%           ovrd = max(log(0.00001)/(log(hnlMin + 1e-10) + 1e-10), 5);
        ...
        hnlLocalMin = min(hnlLocalMin + 0.0008/mult, 1);%更新的意义？
